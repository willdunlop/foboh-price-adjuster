...
export default function Home() {
  ...
  return (
    <div className="w-full h-full px-3 py-6 overflow-auto">
      <Box className="max-w-[1200px] mx-auto bg-slate-50 text-black-grey">
        <ToastContainer />
            ...
            !isEdit
              ? (
                <div className="flex flex-col items-start justify-between sm:items-center sm:flex-row">
                  <DisplayItem
                    heading={`You've selected ${selectedProducts.length} products`}
                    title={priceProfile?.title}
                    subHeading={`With price adjustment mode set to ${profileFormValues.adjustmentType} ${profileFormValues.adjustmentMode} and expires in 16 days`}
                  />
                  <Button variant="text" className="flex gap-2 px-0" onClick={() => setIsEdit(true)}>
                ...
                <>
                  <div className="w-full pt-6 border-b border-slate-200">
                    ...
                    <div className="flex flex-col gap-4 my-3 sm:flex-row sm:items-center">
